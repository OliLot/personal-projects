...
windows();gelman.plot(combinedchains, autoburnin = TRUE)
...
combinedchains <- mcmc.list(posterior$samples$chain1,
                            posterior$samples$chain2)
quartz()# or windows() on a Windows machine
plot(combinedchains)
posterior$summary$all.chains
#
effectiveSize(combinedchains)
...
#
####################################################
###3RD WAY TO OBTAIN POSTERIOR PRED DIST OF X.NEW
####################################################
#
# 
post_lambda=as.matrix(combinedchains[,"lambda"])    
postpred_xnew=rexp(length(post_lambda),post_lambda)
#
#quartz()
#plot(postpred_xnew)
#plot(density(postpred_xnew))
#lines(density(as.matrix(combinedchains[,"x.new"])),col="red")