...
df_mu_c_start = [df0 mu0 c0]; % print out the starting values
...
TOL=tol; MAXCNT=100; conv=0; cnt=0; opts=optimset('disp','none','TolX', tol);
while (conv<1) && (cnt<MAXCNT)
  ...
  if usetrim~=1 %% use the root solve to get mu hat
    mu1star = fzero(@titer_mu,[-50,50],opts,df1,w);
  ...
  conv = disc < TOL; % results are not changing
